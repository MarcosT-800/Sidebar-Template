...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapState(SidebarMenu, ["menu"]),
  },
  data: () => ({
    notificationData: [],
  }),
  methods: {
    ...mapActions(AppOptions, ["appSidebarMobileToggledB"]),
          ...
          elm.onmouseenter = function () {
            ...
            if (appElm && appElm.classList.contains("app-sidebar-minified")) {
              ...
              if (targetMenuHtml) {
                ...
                this.handleSidebarMinifyFloatMenuClick();
              } else {
  ...
  mounted() {
    ...
    var submenuBaseSelector = " > .  > .menu-item.has-sub";
    ...
    this.handleSidebarMinifyFloatMenu();
  },
});
</script>
...
<template>
  <div id="sidebar" class="app-sidebar">
    <perfect-scrollbar class="app-sidebar-content">
      <div class="menu">
        <template v-for="m in menu">
          <div class="menu-header" v-if="m.is_header">{{ m.text }}</div>
          <div class="menu-divider" v-else-if="m.is_divider"></div>
          <template v-else>
            <sidebar-nav v-if="m.text" :menu="m"></sidebar-nav>
          </template>
        </template>
        <div class="p-3 px-4 mt-auto hide-on-minified">
          <a
            href="https://seantheme.com/vue-studio/documentation/index.html"
            target="_blank"
    ...
    <button
      ...
      @click="appSidebarMobileToggledB"
    ></button>
  </div>
</template>
